var alphabet = "abcdefghijklmnopqrstuvwxyz";//inglise tähestik




function OTPencrypt() {
	var sisend = gid("sisend").value.split('');//sisendi väärtus võetakse ja tehakse chararrayks
	var voti = gid("voti").value.split('');//võtme väärtus võetakse ja tehakse chararrayks
	var ciphertext = '';//tagastatud vastus on muutujaga ciphertext

	if (voti.length != sisend.length) {//kui sisend ja võti ei ole sama pikad siis viskab alert akna
		alert("Sorri, aga mul oleks vaja, et võti oleks sama pikk kui sisend. Aitäh!");
	} else {

		for (var i = 0; i < sisend.length; i++) {//läbitakse for tsükkel nii palju kordi kui palju on tähti sõnas
			var sisendCharVal = alphabet.indexOf(sisend[i]);//tähe sisendi kohal i asukoht tähestikus 
			var votiCharVal = alphabet.indexOf(voti[i]);//tähe võtme kohal i asukoht tähestikus 
...
=  4 (E)  16 (Q)  13 (N)  21 (V)  25 (Z) (message + key) mod 26
      ...
      */
	gid("vastus").innerText = "";//kustutab vastus id's teksti 
	gid("vastus").innerText = ciphertext;//vastus id's olev kast saab uue väärtuse

}

//OTPdecrypt on sarnane encryptiga, aga  selle asemel et liita võtmed, siis sisendist lahutatakse võti ja
//kui vastus on negatiivne siis liidetakse sellele tähestikus olevate tähtede arv 26
function OTPdecrypt() {
	var sisend = gid("sisend").value.split('');
	var voti = gid("voti").value.split('');
	var ciphertext = '';

	if (voti.length != sisend.length) {
		alert("Sorri, aga mul oleks vaja, et võti oleks sama pikk kui sisend. Aitäh!");
	} else {
...
=   7 (H)   4 (E)  11 (L)  11 (L)  14 (O) ciphertext – key (mod 26)
       H       E       L       L       O  → message
	*/
	gid("vastus").innerText = "";
	gid("vastus").innerText = ciphertext;

}

/**

SHA-2 - nõuab sisendit ja väljastab vastava SHA-512 tulemuse Base64 kujul
c672b8d1ef56ed28ab87c3622c5114069bdd3ad7b8f9737498d0c01ecef0967a

*/
function SHA2() {
	var sisend = gid("sisend").value; //sisend saab sisendi kastis oleva väärtuse
	console.log(sisend);
	if(sisend.length == 0) { //kui sisendisse ei ole midagi kirjutatud, siis ei ole midagi encryptida ja viskab alert akna
		alert("Sorri, aga ei saa sisestage ka midagi");
	} else {
		var sha2 = SHA512(sisend);//sisend krüpteeritakse SHA512 kujul

		// Järgnev funktsioon window.btoa viib krüpteeritud sisendi base64 kujule ja info selle kohta sain järgnevalt lingilt
		// https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/btoa
		var encodedData = window.btoa(sha2);
		gid("vastus").innerText = encodedData; // Vastus saab nüüd sha512 base64 kujul väärtuse
	}
...
//SHA512 funktsioon
//Järgmise Ploki Allikas: https://coursesweb.net/javascript/sha512-encrypt-hash_cs
